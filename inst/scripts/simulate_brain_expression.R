...
# 
# Inputs:
# - transcripts fasta annotation
# - transcripts annotation sqlite
# - cortexTpmFile
# - mean_disp_fp
# - fit_par_fp
...
ngenes <- as.numeric(args[6])
set.seed(seed)

fasta.fp <- args[7]
txdb.fp <- args[8]      # sqlite file
cortexTpm.fp <- args[9]
meanDisp.fp <- args[10] # dispersion
fitPar.fp <- args[11]   # gc bias params

output.txps.fp <- args[12]
output.expr.fp <- args[13]

# cortex_tpm <- read.delim("../frontal_cortex.tsv.gz", skip=2)
cortex_tpm <- read.delim(cortexTpm.fp, skip=2)
...
names(quant.len) <- quants$Name

# JF Note: Commenting code block factored out
...
txseq <- readDNAStringSet(fasta.fp)
...
### Genes with most transcripts
gMs <- names(table(quants$GENEID))[order(table(quants$GENEID),decreasing=T)]
exprs.genes.M <- gMs[!is.na(match(gMs, exprs.genes))][1:ngenes] ## expressed genes that also contain large txps
tMs <- quants$Name[quants$GENEID %in% exprs.genes.M] ## transcripts belonging to those genes

dge.genes <- sample(exprs.genes.M, length(exprs.genes.M) * dge.ratio)
dte.genes <- sample(setdiff(exprs.genes.M, dge.genes), length(exprs.genes.M) * dte.ratio)
# exclude single isoform genes for DTU
exprs.genes.multi <- setdiff(exprs.genes.M, txdf$GENEID[txdf$ntx == 1])
...
quant.tpm <- quant.tpm[tMs]
quant.len <- quant.len[tMs]
...
if (!onlyDTU) {
  ...
  for (i in seq_along(dge.genes)) {
    if (i %% 100 == 0) cat(i)
    this.txs <- txdf$TXNAME[txdf$GENEID == dge.genes[i]]
    ...
    fc <- runif(1, fc.min, fc.max) # set FCs
    if (coinflip) {
  ...
  for (i in seq_along(dte.genes)) {
    if (i %% 100 == 0) cat(i)
    this.txs <- txdf$TXNAME[txdf$GENEID == dte.genes[i]]
    tpm.this.txs <- quant.tpm[this.txs]
...
per.nuc <- tpms * pmax(quant.len - frag.len, 1)
...
writeXStringSet(txseq, output.txps.fp)
